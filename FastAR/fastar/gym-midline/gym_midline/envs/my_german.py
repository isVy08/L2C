...
class MyGerman(gym.Env):
    metadata = {"render.modes": ["human"]}
    """ A custom OpenAI gym for the reduced version of German Credit dataset """

    def __init__(self, model_id):
        super(MyGerman, self).__init__()
        clf, X_train, X_val, X_test, encoded_columns, immutable_features = classifier.load_model_german(model_id)
        ...
        try:
            self.undesirable_x = np.load(
                f"{os.path.dirname(os.path.realpath(__file__))}/../../../datapoints_to_generate_cfes/undesirable_x_mygerman.npy"
            )
            print("Found")
        except:
            undesirable_x = []
...
class MyGerman1(MyGerman):
    def __init__(self, enable_render=True):
        super(MyGerman1, self).__init__(model_id=1)

class MyGerman2(MyGerman):
    def __init__(self, enable_render=True):
        super(MyGerman2, self).__init__(model_id=2)

class MyGerman3(MyGerman):
    def __init__(self, enable_render=True):
        super(MyGerman3, self).__init__(model_id=3)

class MyGerman4(MyGerman):
    def __init__(self, enable_render=True):
        super(MyGerman4, self).__init__(model_id=4)

class MyGerman5(MyGerman):
    def __init__(self, enable_render=True):
        super(MyGerman5, self).__init__(model_id=5)
