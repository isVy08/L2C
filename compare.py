...
def generate_baseline(X_test, dg, cls, explainer, method, num_samples, max_iters=10000):  
  ...
  N = X_test.shape[0]
  start = time.time()
  features_to_vary = [col for col in X_test.columns if col not in dg.immutable_cols]
  finals = []
  for i in tqdm(range(N)):
    ...
    if method in ('dice', 'copa'):
      ...
      output = explainer.generate_counterfactuals(x, num_samples, "opposite", max_iters)
      output = output.final_cfs_df
    
    elif method == 'certifai':
      fixed = [X_test.columns.tolist().index(col) for col in dg.immutable_cols]
    ...
    if method in ('dice', 'copa'):
        ...
        if method == 'copa':
            ...
            for cls_index in range(5):
                weights = np.hstack((classifiers[cls_index].intercept_, classifiers[cls_index].coef_.squeeze()))
        ...
        for cls_index in range(5):
            ...
            if method == 'dice':    
                ...
                explainer = DroDicePGDAD(dice_data, dice_model, mean_weights=mu_hat, cov_weights=Sigma_hat, verbose=False, max_iter=max_iter)
              
            finals = generate_baseline(X_test, dg, cls, explainer, method, num_samples, params[name]) 
            print(f'Start evaluating {method} at black-box {cls_index + 1} ...')
            evaluate_baseline(X_test, finals, num_samples, dg, cls, knn)
            
    elif method == 'fvae':
        
        dice_data = dice_ml.Data(dataframe=X_train, continuous_features=dg.encoded_columns, outcome_name=dg.target_col)
        backend = {'model': 'base_model.BaseModel',
                'explainer': 'feasible_base_vae.FeasibleBaseVAE'}

        
        params = {'german': [10, 42], 'admission': [30, 62], 'sba': [70, 42], 'student': [10, 62]}
        num_samples = 100
        for cls_index in range(5):
            cls = classifiers[cls_index]
            ...
            explainer = dice_ml.Dice(dice_data, dice_model, encoded_size=params[name][0], validity_reg=params[name][1], margin=0.165, epochs=500)
            ...
            finals = generate_baseline(X_test, dg, cls, explainer, 'FVAE', num_samples, max_iter) 
            print(f'Start evaluating {method} at black-box {cls_index + 1} ...')
            evaluate_baseline(X_test, finals, num_samples, dg, cls, knn)
        ...
        def run_mcce(X_test, dg, cls, num_samples, backend='sklearn'):
            ...
            for col in dg.num_cols:
                dtypes[col] = 'float'
            dataset = Data(dg.df.iloc[:dg.train_size, ], feature_order, dtypes, dg.target_col,  fixed_features, "OneHot", None)
        ...
        for cls_index in range(5):
            start = time.time()
            finals = run_mcce(X_test, dg, classifiers[cls_index], num_samples, 'sklearn')
            print(f'Start evaluating {method} at black-box {cls_index + 1} ...')
            evaluate_baseline(X_test, finals, num_samples, dg, cls, knn)
        ...
        for cls_index in range(5):
            ...
            finals =  generate_baseline(X_test, dg, cls, explainer, method, num_samples, max_iter)
            print(f'Start evaluating {method} at black-box {cls_index + 1} ...')
            evaluate_baseline(X_test, finals, num_samples, dg, cls, knn)
