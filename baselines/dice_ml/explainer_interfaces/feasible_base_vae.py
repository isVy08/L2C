...
class FeasibleBaseVAE(ExplainerBase):
    ...
    def generate_counterfactuals(self, query_instance, total_CFs, desired_class="opposite", limit_step_ls=10000):
        ...
        for i in range(len(query_instance)):
            ...
            for _ in range(total_CFs):
                ...
                while (cf_label == train_y and iter < limit_step_ls):
        ...
        final_cfs_df = pd.DataFrame(cfs, columns=self.data_interface.continuous_feature_names)
        final_cfs_df[self.data_interface.outcome_name] = np.array(cfs_preds)
        return final_cfs_df
        
        
        # test_instance_df = result['query-instance'] # self.data_interface.get_inverse_ohe_min_max_normalized_data(result['query-instance'])
